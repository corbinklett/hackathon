...
def parse_body(fd, buf):
    ...
    fd["ies"] = ies
    data = ""
    #for _ in range(24 + fixed[fd["subt"]],  fd["len"]):
    #    data = data + chr(buf[_])
    #fd["data"] = data
    fd["data"] = "".join("%02X" % _ for _ in buf[24 + fd["subt"] : fd["len"]])
    return fd
...
monitor = wifi.Monitor(queue=256)
loop = 0
packets = 0
    ...
    try:
        ...
        if len(fd) > 2:
            ...
            if PARSE_BODY:
                ...
                if fd["typename"] == "data" and fd["subtname"] == "AssocReq":
                    print(f'{fd["data"]}', end=" ")
                ie_ids = []
                for _ in fd["ies"]:
